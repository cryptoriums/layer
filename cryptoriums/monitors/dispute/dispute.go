...
func NewMonitor(logger log.Logger, cfg Config) *Monitor {
	...
	for {
		...
		case <-ticker.C:
			...
			for _, node := range cfg.Nodes {
				...
				go func(node string) {
					defer wg.Done()
					result, err := m.queryDisputes(node)
					if err != nil {
...
func (m *Monitor) queryDisputes(node string) (*types.QueryOpenDisputesResponse, error) {
	...
	client := &http.Client{Timeout: 5 * QueryInterval}
