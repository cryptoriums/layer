...
// messages with the local key. sign() hashes once more, so each signature is
// over sha256(sha256(message)) - the same digest the remote signer produces.
func (s *KeyringSigner) SignInitialRegistration(_ context.Context, operatorAddress string) ([]byte, []byte, error) {
