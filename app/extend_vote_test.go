	...
	testCases := []testCase{
		{
			...
			setupMocks: func(ok *mocks.OracleKeeper, bk *mocks.BridgeKeeper, signer *mocks.VoteExtensionSigner, h *app.VoteExtHandler, patches *gomonkey.Patches) {
				...
				signer.On("SignInitialRegistration", mock.Anything, oppAddr).Return(nil, nil, errors.New("error!")).Once()
			},
			expectedPanic: false,
		...
		},
		{
			...
			setupMocks: func(ok *mocks.OracleKeeper, bk *mocks.BridgeKeeper, signer *mocks.VoteExtensionSigner, h *app.VoteExtHandler, patches *gomonkey.Patches) {
				signer.On("GetOperatorAddress", mock.Anything).Return(oppAddr, nil)
				bk.On("GetEVMAddressByOperator", ctx, oppAddr).Return(nil, collections.ErrNotFound)
				signer.On("SignInitialRegistration", mock.Anything, oppAddr).Return([]byte("sigA"), []byte("sigB"), nil).Once()
		...
		},
		{
			...
			setupMocks: func(ok *mocks.OracleKeeper, bk *mocks.BridgeKeeper, signer *mocks.VoteExtensionSigner, h *app.VoteExtHandler, patches *gomonkey.Patches) {
				signer.On("GetOperatorAddress", mock.Anything).Return(oppAddr, nil)
				bk.On("GetEVMAddressByOperator", ctx, oppAddr).Return(nil, collections.ErrNotFound)
				// SignInitialMessage now delegates straight to the signer, which builds
				// both registration messages from the operator address itself.
				signer.On("SignInitialRegistration", mock.Anything, oppAddr).Return([]byte("sigA"), []byte("sigB"), nil).Once()
...
func (s *VoteExtensionTestSuite) TestSignInitialMessage() {
	...
	testCases := []struct {
	...
	}{
		{
			name: "success",
			setupSigner: func(signer *mocks.VoteExtensionSigner) {
				signer.On("SignInitialRegistration", mock.Anything, operatorAddr).
					Return([]byte("signedMsgA"), []byte("signedMsgB"), nil).Once()
			},
			expectedSigA: []byte("signedMsgA"),
			expectedSigB: []byte("signedMsgB"),
		},
		{
			name: "signer error is propagated",
			setupSigner: func(signer *mocks.VoteExtensionSigner) {
				signer.On("SignInitialRegistration", mock.Anything, operatorAddr).
					Return(nil, nil, errors.New("sign failed")).Once()
			},
